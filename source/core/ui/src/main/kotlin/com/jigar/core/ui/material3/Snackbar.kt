...
package com.jigar.core.ui.material3
...
import com.jigar.core.ui.token.SnackbarTokens
...
fun Snackbar(
    ...
    Snackbar(
        ...
        leadingIcon = if (snackbarData.visuals.type == null) null else {
            {
                val type = snackbarData.visuals.type!!
...
private fun OneRowSnackbar(
    ...
    val leadingIconTag = "leadingIcon"
    val textTag = "text"
    ...
    Layout(
        {
            if (leadingIcon != null) {
                Box(Modifier.layoutId(leadingIconTag)) {
                    leadingIcon.invoke()
                }
            }
            Box(
        ...
        val leadingIconPlaceable =
            measurables.fastFirstOrNull { it.layoutId == leadingIconTag }?.measure(constraints)
        ...
        val leadingIconWidthWithPadding = if (leadingIconWidth == 0) 0 else (leadingIconWidth + HorizontalSpacing.toPx()).toInt()
        val textMaxWidth =
            (containerWidth - leadingIconWidthWithPadding - actionButtonWidth - dismissButtonWidth - extraSpacingWidth)
        ...
        val textPlaceX = leadingIconWidthWithPadding
        ...
        layout(containerWidth, containerHeight) {
            leadingIconPlaceable?.placeRelative(0, leadingIconPlaceY)
